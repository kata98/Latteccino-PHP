...
if(!isset($_GET['id'])){
    ...
    $id_up = $_GET['id'];
    $upit_up = "SELECT * FROM menu1 WHERE id = :id_up";

    $rezultat_up = $konekcija->prepare($upit_up);
    $rezultat_up->bindParam(":id_up", $id_up);
    $rezultat_up->execute();
    
    $meni_up = $rezultat_up->fetch(); 
...
<body>
    ...
    <div id="drugi">
        <div id="booking">
            <form action="../models/proizvodi/proveraUpdateProizvod.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="id_up" id="id_up" value="<?= $meni_up->id ?>"/> 
                <input type="text" name="naslov2" placeholder="Product name" value="<?= $meni_up->ime ?>" class="naslov" id="naslov"><br/>
                <input type="text" name="cena2" placeholder="Product price" value="<?= $meni_up->cena ?>" class="cena" id="cena"><br/>
                <input type="text" name="opis2" placeholder="Product description" value="<?= $meni_up->opis ?>" class="opis" id="opis"><br>
                <input type="file" name="slika2" value="" class="slika" id="slika"><br>

                <input class="dugme" type="submit" value="UPDATE ITEM" name="updateU" id="update">
